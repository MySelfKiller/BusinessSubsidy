...
import com.hoom.library.common.state.*
 ...
 *
 */
fun <T> BaseFrameActivity<*,*>.parseState(
 ...
 *
 */
fun <T> BaseFrameFragment<*,*>.parseState(
    resultState: ResultState<T>,
    onSuccess: (T) -> Unit,
    onError: ((AppException) -> Unit)? = null,
 ...
 * @param loadingMessage 加载框提示内容
 */
fun <T> BaseViewModel.reqGaoDe(
    block: suspend () -> ApiWeatherResponse<T>,
    resultState: MutableLiveData<ResultState<T>>,
    isShowDialog: Boolean = false,
    loadingMessage: String = "请求网络中..."
): Job {
    return viewModelScope.launch {
        runCatching {
            ...
            resultState.paresWeatherResult(it)
        }.onFailure {
            it.message?.loge()
            //打印错误栈信息
            it.printStackTrace()
 ...
 * @param loadingMessage 加载框提示内容
 */
fun <T,U> BaseViewModel.request2(
    block: suspend () -> Base2Response<T,U>,
    resultState: MutableLiveData<ResultState<T>>,
    resultState2: MutableLiveData<ResultState<U>>,
    isShowDialog: Boolean = false,
    loadingMessage: String = "请求网络中..."
): Job {
    return viewModelScope.launch {
        runCatching {
            if (isShowDialog) {
                ...
                resultState2.value = ResultState.onAppLoading(loadingMessage)
            }
            //请求体
            block()
        }.onSuccess {
            resultState.paresResultT(it)
            resultState2.paresResultU(it)
        }.onFailure {
            it.message?.loge()
            //打印错误栈信息
            it.printStackTrace()
            resultState.paresException(it)
            resultState2.paresException(it)
        }
    }
}
...
fun <T> BaseViewModel.requestNoCheck(
    ...
    return viewModelScope.launch {
        runCatching {
            ...
            loadingChange.dismissDialog.postValue(false)
            //成功回调
            LogUtil.e("requestNoCheck   ",it.toString())
            success(it)
        }.onFailure {
            //网络请求异常 关闭弹窗
            loadingChange.dismissDialog.postValue(false)
    ...
    coroutineScope {
        when {
            response.isSucces() -> {
                ...
                if (response.getResponseCode() == Constants.response_code_10101 || response.getResponseCode() == Constants.response_code_10102){
                    val intent = Intent("com.kayu.business.subsidy.JUMP")
                    BaseApplication.context.sendBroadcast(intent)
                    LogUtil.e("executeResponse","发送需要登录广播${intent.data}")
                }
                throw AppException(
