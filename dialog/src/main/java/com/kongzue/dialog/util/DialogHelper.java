                    ...
                    if (dialog != null) {
                        ...
                        if (dialogWindow != null) {
                            if (isWaitAddFocusFlag) {
                                ...
                                dialogWindow.getDecorView().setSystemUiVisibility(getSystemUiVisibility());//获取视口全屏大小
                                dialogWindow.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS); //这个flag表示window负责绘制状态栏的背景当设置了这个flag,系统状态栏会变透明,同时这个相应的区域会被填满 getStatusBarColor() and getNavigationBarColor()的颜色
//                                dialogWindow.setStatusBarColor(Color.TRANSPARENT);//设置bar为透明色
//                                WindowManager.LayoutParams layoutParams = dialogWindow.getAttributes();//获取dialog布局的参数
//                                if (getBackgroundDrawable() != null) {
//                                    dialogWindow.setBackgroundDrawable(getBackgroundDrawable());
//                                }
...
//                                dialogWindow.setNavigationBarColor(Color.TRANSPARENT);//设置导航栏颜
//                                dialogWindow.setType(WindowManager.LayoutParams.TYPE_APPLICATION_PANEL);//内容扩展到导航栏
...
//                                dialogWindow.setAttributes(layoutParams);
        ...
        if (dialog != null && parent != null) {
            ...
            switch (parent.get().align) {
                ...
                case RIGHT:
                    ...
                    lp.windowAnimations = R.style.rightDialogAnim;
                    break;
                case DEFAULT:
    ...
    private boolean isShowNavBar(Context context) {
        int x = ((Activity) context).getWindow().getDecorView().getSystemUiVisibility();
        int option = View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE;
        if ( x  == View.SYSTEM_UI_FLAG_FULLSCREEN || x == option) {
            return true;
        }
        return false;
    }
